...
<template>
  <div class="container mx-auto grid min-h-screen grid-cols-1 items-center justify-center gap-8 lg:grid-cols-2">
    <div class="row-start-2 flex items-center justify-center lg:row-start-1">
      <img src="@/assets/thibke.png" alt="Chat" class=" h-[40rem] ">
    </div>
    <div class="flex flex-grow flex-col items-center justify-center gap-2 ">
      <h1 class="mb-4 flex w-full items-center gap-4 text-left text-5xl font-bold">
      ...
      <div class="flex w-full flex-col gap-2 rounded bg-white p-4 shadow-main">
        <!-- <h2 class="text-xl font-bold">
          Chat: {{ currentChat }}
        </h2> -->
        <ChatLog v-if="conversation?.messages" class="max-h-[60vh] overflow-auto" :log="conversation?.messages" :is-loading="isSending" />
